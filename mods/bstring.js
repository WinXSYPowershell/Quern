...
function handleSplit(varName, strVal, tokens) {
    ...
    if (QuernAPI.Runtime) {
        ...
        QuernAPI.Log("Split performed internally (Runtime access unavailable). Parts: " + parts.join(" | "));
    }
}
...
function handleSuString(varName, strVal, tokens) {
    ...
    var result = strVal.suString(startIdx, startIdx + count);
...
function handleTrim(varName, tokens) {
    // Syntax: String "Var" Trim
    // Gets the variable, trims it, and updates it.
    
    var currentVal = QuernAPI.GetVariable(varName);
    if (currentVal === varName) currentVal = ""; // Handle undefined
    ...
    if (QuernAPI.Runtime) {
        try {
            ...
            QuernAPI.Log("Trimmed '" + varName + "'. New value: '" + trimmedVal + "'");
        } catch (e) {
            QuernAPI.Log("Error trimming variable: " + e.message);
        }
    } else {
        QuernAPI.Log("Trimmed result: '" + trimmedVal + "'");
    }
}

function handleLength(varName, strVal, tokens) {
    // Syntax: String "Var" Length
    // Stores the length in a variable "VarName_Len"
    ...
    if (QuernAPI.Runtime) {
        ...
        QuernAPI.Log("Length of '" + varName + "' is " + len);
    }
}
